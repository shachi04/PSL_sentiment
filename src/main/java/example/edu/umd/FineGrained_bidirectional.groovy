...
class FineGrained_bidirectional{
	
	public static void main(String[] args)
	{
	for(int i = 0; i <10; ++i)
	{
					FineGrained_bidirectional a = new FineGrained_bidirectional()
					a.pslmodel(i);
	}
	}
	...
	void pslmodel(int cvSet)
	{
		...
		String writefolder = System.getProperty("user.home") + "/Documents/Shachi/CMPS209C/reviews/Results/bidirectional/"
...
//		m.add rule : negsentiment(A) >> (nrclexiconneg(A) ) , weight :5
		...
		m.add rule : ( all(A) & ~unigrampos(A)) >> ~possentiment(A), weight : 5, squared : false
		m.add rule : ( all(A) & ~unigramneg(A)) >> ~negsentiment(A), weight : 5, squared : false
		...
		m.add rule : ( all(A) & ~tglpos(A)) >> ~possentiment(A), weight : 5, squared : false
		m.add rule : (all(A) & ~tglneg(A)) >> ~negsentiment(A), weight : 5, squared : false
